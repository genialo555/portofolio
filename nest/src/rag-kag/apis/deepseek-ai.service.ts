...
export class DeepseekAiService {
  ...
  async generateResponse(
    ...
    this.logger.info("Envoi d'une requête à Deepseek AI", {
      promptLength: prompt.length,
      options
    });

    // Simulation d'un appel à l'API Deepseek AI
    // À remplacer par un vrai appel API
    const delay = Math.random() * 1800 + 900;
    await new Promise(resolve => setTimeout(resolve, delay));

    const responseText = `Ceci est une réponse simulée de l'API Deepseek AI pour le prompt : "${prompt.substring(0, 50)}..."`;
    ...
    const usage: ApiUsageMetrics = {
      ...
      processingTime,
      cost: Math.ceil((prompt.length + responseText.length) / 4) * 0.000005
    };

    this.logger.info('Réponse reçue de Deepseek AI', { 
