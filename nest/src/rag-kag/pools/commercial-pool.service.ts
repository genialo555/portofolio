...
/**
 * Service gérant le pool d'agents commerciaux
 */
@Injectable()
export class CommercialPoolService {
  ...
  constructor(
    ...
    this.logger = logger;
    this.logger.info('Service de pool commercial initialisé');
  }
  
  /**
   * Exécute les agents du pool commercial sur une requête utilisateur
   * @param query Requête à traiter
   * @returns Résultats des agents commerciaux
   */
  async executeAgents(query: UserQuery): Promise<AgentResult[]> {
    this.logger.info('Exécution des agents commerciaux', { queryId: query.sessionId });
    
    try {
      // 1. Obtenir les agents configurés pour ce pool
      const agents = await this.agentFactory.getAgentsForPool(PoolType.COMMERCIAL);
      
      if (!agents || agents.length === 0) {
        this.logger.warn('Aucun agent commercial disponible');
        return [];
      }
      
      this.logger.debug(`${agents.length} agents commerciaux disponibles`);
      ...
      this.logger.info('Traitement des agents commerciaux terminé', {
      ...
      this.logger.error('Erreur dans l\'exécution du pool commercial', { 
        error
      });
      throw error;
    }
