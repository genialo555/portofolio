...
export function determineRelevantPools(query: string): Record<AgentType, number> {
  ...
  // Termes associés à chaque domaine
  const domainTerms: Record<AgentType, string[]> = {
    [AgentType.COMMERCIAL]: ['vente', 'client', 'prix', 'offre', 'proposition', 'contrat', 'négociation', 'crm'],
    [AgentType.MARKETING]: ['campagne', 'communication', 'publicité', 'marque', 'audience', 'cible', 'promotion', 'réseaux'],
    [AgentType.SECTORIEL]: ['industrie', 'marché', 'secteur', 'concurrence', 'tendance', 'technologie', 'b2b', 'b2c'],
    [AgentType.EDUCATIONAL]: ['apprendre', 'éducation', 'enseignement', 'formation', 'cours', 'pédagogie', 'élève', 'étudiant', 'apprentissage']
  };
  ...
  const relevance: Record<AgentType, number> = {
    ...
    [AgentType.EDUCATIONAL]: 0
  };
  
  // Calcul basique de pertinence basé sur la présence de mots-clés
  Object.entries(domainTerms).forEach(([type, words]) => {
